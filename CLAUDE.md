...
- **Category CRUD MCP tools + hierarchy metadata fix + live-DB cleanup** (2026-06-10): Five new MCP tools: `create_category`, `rename_category`, `move_category`, `delete_category`, `merge_categories`. Root-cause fix for broken hierarchy metadata: `POST /api/categories` (the route the UI calls) went through `accountService.createAccount`, which never computes `fullPath`/`level`/`sortOrder` — it now delegates to `categoryService.createCategory` (which also gained a duplicate-name-under-same-parent check). `updateCategory` now refuses cycle-creating moves and INCOME↔EXPENSE moves, and recursively repairs descendant `fullPath`/`level` on rename/move. New `mergeCategories` (re-points postings, memorized rules, recurring bills, then deletes source; refuses cross-type or source-with-children) and `repairHierarchy` (recomputes level/fullPath from parentId chain, renumbers duplicate sibling sortOrders; skips ASSET/LIABILITY GST system accounts). New endpoints: `POST /api/categories/merge`, `POST /api/categories/repair-hierarchy`. **fullPath convention**: standardized on title-case roots (`Expense/...`, `Income/...`) matching the seeded data; the service previously generated enum-style (`EXPENSE/...`). One-off script `scripts/category-cleanup-2026-06.ts` ran against the live book DB (backup at `ledger.db.bak-2026-06-10-category-cleanup`): repaired 111 paths + 68 sortOrders, promoted "Yuko's Business" → top-level "Yuko Business", cleared stray `isBusiness` flags on 2 Web Central postings, deleted 28 unused template categories, created 3 Bank Fees children (Credit Card Interest, Account Fees, Penalty Fees). 16 new unit tests. Files: `src/lib/services/categoryService.ts`, `src-server/api.ts`, `src-server/validation.ts`, `src-mcp/index.ts`, `src-mcp/api-client.ts`, `scripts/category-cleanup-2026-06.ts`, `scripts/inspect-categories.ts`.
